...
def configure(conf):
    ...
    # @@added
    import os
    AIRSIM_ROOT = os.getcwd() + '/../../../AirSim'
    RPCLIB_VERSION_FOLDER = os.listdir(f'{AIRSIM_ROOT}/external/rpclib/')[0]
    print(f'using AIRSIM_ROOT as {AIRSIM_ROOT}')
    print(f'using RPCLIB_VERSION_FOLDER as {RPCLIB_VERSION_FOLDER}')
...
def create_ns3_program(bld, name, dependencies=('core',)):
    ...
    # program = bld(features='cxx cxxprogram')
    # -----------------------------------------
    program = bld(features='cxx cxxprogram', lib=['AirLib', 'MavLinkCom', 'rpc', 'zmq'])
    # @@ switch
    program.is_ns3_program = True
    program.name = name
    program.target = "%s%s-%s%s" % (wutils.APPNAME, wutils.VERSION, name, bld.env.BUILD_SUFFIX)
